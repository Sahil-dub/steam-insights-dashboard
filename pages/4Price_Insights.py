...
st.markdown("---")
st.markdown("### 🎯 Value-for-Money Games (Playtime per €)")

# Filter games with price > 0 and playtime > 0
value_df = price_df[(price_df['price'] > 0) & (price_df['average_playtime_forever'] > 0)].copy()

value_df['value_score'] = value_df['average_playtime_forever'] / value_df['price']

# Top 10 value-for-money games
top_value = value_df.sort_values(by='value_score', ascending=False).head(10)

st.dataframe(
    top_value[[
        "name", "price", "average_playtime_forever", "value_score"
    ]].rename(columns={
        "name": "🎮 Game",
        "price": "💵 Price (€)",
        "average_playtime_forever": "🕹️ Playtime (min)",
        "value_score": "✨ Minutes per €"
    }),
